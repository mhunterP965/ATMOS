...
# -------- mechanisms (BOTTOM boundary) — readability cleanup --------
# M1 shared bus (y10.3, x3.0..13.0) with one stub to the boundary and 3 clean risers.
# Local M11/M12/M13 rise from the boundary on their own anchors; only M12 crosses the bus.
path([(8.0, 11.8), (8.0, 10.3)], arrow=False)            # M1 stub from boundary -> bus
path([(3.0, 10.3), (13.0, 10.3)], arrow=False)           # M1 shared bus
path([(3.0, 10.3), (3.0, 4.4)])                          # M1 -> A11 bottom
path([(4.3, 10.3), (4.3, 8.8)])                          # M1 -> A12 bottom
path([(13.0, 10.3), (13.0, 6.6)])                        # M1 -> A13 bottom
path([(2.8, 11.8), (2.8, 4.4)])                          # M11 -> A11 bottom (left of bus)
path([(5.3, 11.8), (5.3, 8.8)])                          # M12 -> A12 bottom
path([(14.0, 11.8), (14.0, 6.6)])                        # M13 -> A13 bottom (right of bus)
label(8.2, 9.95, 3.3, 0.3, "M1 Platforms hosting ATMOS node compute", size=9, align=PP_ALIGN.CENTER)
label(1.4, 11.9, 2.6, 0.4, "M11 Onboard/attached sensors", size=9, align=PP_ALIGN.CENTER)
label(4.0, 11.9, 2.9, 0.42, "M12 DDS middleware; radios/network transport", size=9, align=PP_ALIGN.CENTER)
label(12.0, 11.9, 3.6, 0.42, "M13 ATMOS edge processing; navigation solution (GPS/INS); local compute", size=9, align=PP_ALIGN.CENTER)

prs.save("ATMOS_A1_native_working_v2.pptx")
print("saved ATMOS_A1_native_working_v2.pptx; shapes:", len(slide.shapes))
