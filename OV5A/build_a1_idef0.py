...
# ============================ MECHANISMS (bottom) ============================
BUSY = 6.90
seg(1.45,BUSY,7.80,BUSY)                          # mechanism bus
# M1 -> A12, A13, A11(detour)
path([(2.50,BUSY),(2.50,5.85)])                   # M1 -> A12 bottom
path([(6.80,BUSY),(6.80,4.60)])                   # M1 -> A13 bottom
path([(4.05,BUSY),(4.05,4.30),(2.50,4.30),(2.50,3.20)])  # M1 -> A11 bottom
# M11 -> A11 (left detour)
path([(1.45,BUSY),(1.45,4.10),(3.20,4.10),(3.20,3.20)])  # M11 -> A11 bottom
# M12 -> A12
path([(3.20,BUSY),(3.20,5.85)])                   # M12 -> A12 bottom
# M13 -> A13
path([(7.80,BUSY),(7.80,4.60)])                   # M13 -> A13 bottom
add_label('M1 Platforms hosting ATMOS node compute',1.70,7.02,2.20,0.50,10, fill_white=True)
add_label('M11 Onboard/attached sensors',0.55,7.02,1.10,0.55,10, fill_white=True)
add_label('M12 DDS middleware; radios/network transport',4.10,7.02,2.10,0.50,10, fill_white=True)
add_label('M13 ATMOS edge processing; navigation solution (GPS/INS); local compute',
          6.45,7.00,2.55,0.58,10, fill_white=True)

prs.save('ATMOS_A1_IDEF0_native_vNext.pptx')
