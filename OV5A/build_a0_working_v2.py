...
box(17.8, 3.0, 3.0, 1.3, "Detect Threshold Crossings (Descriptive Support)", "A5")
box(17.8, 7.4, 3.0, 1.3, "Produce Mission-Tailored Weather Context", "A6")

# A5: left 17.8, right 20.8, top-mid 19.3, bottom-mid 19.3; left-mids 3.65/4.05
# A6: left 17.8, right 20.8, top 18.9/19.6 (y7.4), bottom 18.9/19.6 (y8.7); left-mids 7.85/8.25

# -------- internal flows F1-F5 --------
path([(5.0, 5.85), (5.7, 5.85)])
path([(8.3, 5.85), (9.0, 5.85)])
path([(11.6, 5.85), (12.3, 5.85)])
path([(15.1, 5.55), (15.6, 5.55), (15.6, 3.65), (17.8, 3.65)])    # F4 -> A5 left
path([(15.1, 6.15), (15.6, 6.15), (15.6, 7.85), (17.8, 7.85)])    # F5 -> A6 left
...
label(13.8, 2.30, 2.3, 0.45, "F4 Federated Weather Context / COWP State", size=9)
label(13.0, 6.86, 1.55, 0.80, "F5 Federated Weather Context / COWP State", size=9)

# -------- inputs (LEFT boundary; I3 bus kept high above the mechanism buses) --------
path([(0.5, 5.55), (2.4, 5.55)])           # I1 -> A1
path([(0.5, 6.15), (2.4, 6.15)])           # I2 -> A1
path([(0.5, 9.4), (16.0, 9.4)], arrow=False)            # I3 bus (y9.4, separate from mech buses 10.5/10.9/11.1)
path([(16.0, 9.4), (16.0, 4.05), (17.8, 4.05)])         # I3 -> A5
path([(16.0, 9.4), (16.0, 8.25), (17.8, 8.25)])         # I3 -> A6
...
label(0.6, 9.05, 2.9, 0.34, "I3 Mission Weather Threshold Definitions", size=9)

# -------- controls (TOP boundary; A6 branches routed down the A4/stack gap, off the output lanes) --------
path([(13.3, 1.1), (13.3, 5.2)])                                       # C3 -> A4
path([(14.2, 1.1), (14.2, 1.4), (16.5, 1.4)], arrow=False)            # C2 trunk
path([(14.2, 1.4), (14.2, 5.2)])                                       # C2 -> A4
path([(16.5, 1.4), (16.5, 6.9), (18.9, 6.9), (18.9, 7.4)])           # C2 -> A6
path([(19.3, 1.1), (19.3, 1.5), (17.4, 1.5)], arrow=False)            # C1 trunk
path([(19.3, 1.5), (19.3, 3.0)])                                       # C1 -> A5
path([(17.4, 1.5), (17.4, 7.1), (19.6, 7.1), (19.6, 7.4)])           # C1 -> A6
...
# -------- outputs (to RIGHT boundary; separate lanes) --------
path([(15.1, 5.85), (21.5, 5.85)])         # O1 from A4 (middle lane, passes between F4/F5)
path([(20.8, 3.65), (21.5, 3.65)])         # O3 from A5
path([(20.8, 8.05), (21.5, 8.05)])         # O2 from A6
...
path([(3.7, 10.5), (19.6, 10.5)], arrow=False)          # M1 bus
path([(3.7, 10.5), (3.7, 6.5)])                         # M1 -> A1
path([(6.6, 10.5), (6.6, 6.5)])                         # M1 -> A2
path([(9.9, 10.5), (9.9, 6.5)])                         # M1 -> A3
path([(12.8, 10.5), (12.8, 6.5)])                       # M1 -> A4
path([(19.0, 10.5), (19.0, 8.7)])                       # M1 -> A6
path([(17.0, 10.5), (17.0, 5.9), (19.3, 5.9), (19.3, 4.3)])   # M1 -> A5 (around A6 via gap)
# M2 -> A2, A3
...
# M3 -> A4, A6  (bottom perimeters only)
path([(14.8, 11.8), (14.8, 11.1)], arrow=False)         # M3 stub
path([(14.6, 11.1), (19.6, 11.1)], arrow=False)         # M3 bus
path([(14.6, 11.1), (14.6, 6.5)])                       # M3 -> A4
path([(19.6, 11.1), (19.6, 8.7)])                       # M3 -> A6
...
prs.save("ATMOS_A0_native_working_v2.pptx")
print("saved ATMOS_A0_native_working_v2.pptx; shapes:", len(slide.shapes))
